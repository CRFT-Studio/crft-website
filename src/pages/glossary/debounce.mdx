...
Imagine a search box that queries an [API](/glossary/api) on every keystroke. Without debouncing, typing "website" could trigger seven requests. With a 300ms debounce, the request fires once after the user pauses typing.
...
- Lower CPU work on the [main thread](/glossary/main-thread)
- Smoother [UI](/glossary/user-interface) under rapid input
- Reduced [backend](/glossary/backend) load

## Implementation Tips

1. Choose delays based on [UX](/glossary/user-experience), often 150–400ms for search
2. Decide whether leading-edge execution is needed
3. Cancel pending debounced work on unmount in [SPA](/glossary/single-page-application) components
...
### Stale Closures
In reactive UI [frameworks](/glossary/framework), debounced callbacks can capture outdated state if not handled carefully.

### [Accessibility](/glossary/accessibility) and Expectations
