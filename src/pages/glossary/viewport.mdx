...
## Role in [Responsive Design](/glossary/responsive-design)

The viewport plays a fundamental role in responsive web design through:

### Media Queries
Developers use viewport-based media queries to create [breakpoints](/glossary/breakpoints) that trigger different layouts based on screen size:
...
### Viewport Units
Modern [CSS](/glossary/css) provides viewport-relative units:
- `vw` (viewport width)
- `vh` (viewport height)
...
1. **Mobile [Optimization](/glossary/optimization)**: Ensuring content displays correctly across various mobile device viewports
