...
GraphQL is an API style where clients describe the shape of data they need, and the server responds with matching [JSON](/glossary/json). Instead of multiple fixed [endpoints](/glossary/endpoint), many GraphQL [APIs](/glossary/api) expose a single endpoint with a typed schema of queries, mutations, and subscriptions.
...
New [UI](/glossary/user-interface) requirements can often be met by adjusting queries without new endpoints.
...
- [Caching](/glossary/cache) is different from traditional [HTTP](/glossary/http) resource caching
...
3. Use [pagination](/glossary/pagination) for list fields
